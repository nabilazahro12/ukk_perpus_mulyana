...
function checkIsNotLogin()
{
	if (!isset($_SESSION['login'])) {
		header("Location:http://localhost/ukk_perpus_mulyana/login");
	}
}

function urlTo($to)
{
	return 'http://localhost/ukk_perpus_mulyana'.$to;
}

function redirectTo($icon, $pesan, $tujuan)
{
	setcookie('alert', serialize([$icon, $pesan]), time() + 1, '/');
	header("Location:http://localhost/ukk_perpus_mulyana".$tujuan);
}
